...
class DataSampler():
    ...
    def _process_file_valid_samples(self, h5_path, area_idx, file_idx):
        with h5py.File(h5_path) as f:
            ...
            depth_labels = np.array(f['depth_labels'])
            ...
            for idx, (ts, label, edge_idx) in enumerate(zip(depth_timestamps, depth_labels, depth_edge_idxs)):
                sampled_img_idxs = self._is_valid_sample(idx, label, ts, depth_timestamps)
    ...
    def process_dataset(self, data_dir):
        ...
        for area_idx, area_dir in enumerate(area_keys):
            ...
            for file_idx, file in enumerate(area_files):
                ...
                curr_file_valid_labels = [label for _, label, _, _ in file_valid_samples]
        ...
        for img_idxs, label, edge_idx, file_desc in valid_samples:
            curr_behaviour_idx = behaviour_idxs[label]

            if len(samples[label]) > 0 and samples[label][-1] == curr_behaviour_idx:
                samples[label].pop()
